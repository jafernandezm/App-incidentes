                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <div class="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
                        ...
                        <ul class="list-disc list-inside space-y-2 text-gray-600">
                            ...
                            <li><span class="font-medium">Servidor:</span> {{ $detalles['HTTPServer'] ?? 'Desconocido' }}
                            </li>
                            <li><span class="font-medium">CMS:</span> {{ $cmsText }}</li>

                            <li><span class="font-medium">Fecha del escaneo:</span>
                                {{ \Carbon\Carbon::parse($escaneo['fecha'])->format('D Y/m/d ') }}</li>
                        </ul>
                    </div>
                    ...
                    <div class="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
                        ...
                        <ul class="list-disc list-inside space-y-2 text-gray-600">
                            <li><span class="font-medium">Archivos analisados:</span> {{ $detalles['cantidad'] ?? '0' }}
                            </li>
                            ...
                            @forelse ($escaneo->resultados as $resultado)
                                <li>
                                    ...
                                    {{ $resultado->detalle ?? 'No se encontraron detalles' }}
                                </li>
                            @empty
                                <li>No se encontraron resultados</li>
                            @endforelse
                            <div class="mt-4">
                                <a href="{{ route('escaneo.show', ['id' => $escaneo->id]) }}" class="inline-block bg-blue-500 text-white py-2 px-4 rounded hover:bg-blue-600">
                                    Ver
                                </a>
                            </div>
                            </li>
                    ...
                    <div class="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
                        ...
                        <ul class="list-disc list-inside space-y-2 text-gray-600">
                            <li><span class="font-medium">Quttera Labs:</span>
                                {{ $escaneo['estado_lista_negra']['quttera'] ?? 'Desconocido' }}</li>
                            <li><span class="font-medium">Zeus Tracker:</span>
                                {{ $escaneo['estado_lista_negra']['zeus'] ?? 'Desconocido' }}</li>
                        </ul>
                    </div>
                </div>
            </div>
